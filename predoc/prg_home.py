...
def precision_gain(tp, fn, fp, tn):
    ...
    n_pos = tp + fn
    n_neg = fp + tn
    prec_gain = 1.0 - (n_pos / (n_neg + epsilon)) * (fp / (tp + epsilon))
...
def recall_gain(tp, fn, fp, tn):
    ...
    n_pos = tp + fn
    n_neg = fp + tn
    recall_gain = 1.0 - (n_pos / (n_neg + epsilon)) * (fn / (tp + epsilon))
...
def test():
    labels = np.array(
        [1, 1, 1, 0, 1, 1, 1, 1, 1, 1, 0, 1, 1, 1, 0, 1, 0, 0, 1, 0, 0, 0, 1, 0, 1],
        dtype="int",
    )
...
def get_scores(df, recall, precision):
    D = {}

    for i in range(0, len(df) - 1, 2):
        rec1 = df.iloc[i][recall]
        rec2 = df.iloc[i + 1][recall]

        prec1 = df.iloc[i][precision]
        prec2 = df.iloc[i + 1][precision]
...
def get_betas(scores):
    ...
    for score in s:
        try:
            beta[score] = sqrt(abs((1 - float(score)) / float(score)))
        except:
            print("Division by 0")
